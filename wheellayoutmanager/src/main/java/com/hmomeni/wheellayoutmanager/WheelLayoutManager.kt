...
class WheelLayoutManager : RecyclerView.LayoutManager() {
	private val TAG = WheelLayoutManager::class.java.simpleName
	...
	override fun onLayoutChildren(recycler: RecyclerView.Recycler?, state: RecyclerView.State?) {
		...
		if (childCount == 0) { //First or empty layout
			...
			childTop = paddingTop
		} else if (!state.isPreLayout && getVisibleChildCount() >= state.itemCount) {
			...
			val isOutOfRowBounds = getFirstVisibleRow() > maxFirstRow
			val isOutOfColBounds = getFirstVisibleColumn() > maxFirstCol
	...
	private fun fillGrid(direction: Int, emptyLeft: Int, emptyTop: Int,
		...
		when (direction) {
			...
			DIRECTION_UP -> mFirstVisiblePosition -= getTotalColumnCount()
			DIRECTION_DOWN -> mFirstVisiblePosition += getTotalColumnCount()
		}
		...
		for (i in 0 until getVisibleChildCount()) {
			...
			if (state!!.isPreLayout) {
				...
				(0 until removedPositions!!.size())
						.filter {
							//Look for off-screen removals that are less-than this
							removedPositions.valueAt(it) == REMOVE_INVISIBLE && removedPositions.keyAt(it) < nextPosition
						}
						.forEach {
     ...
     */
	override fun canScrollHorizontally(): Boolean = //We do allow scrolling
			true
	...
	override fun scrollHorizontallyBy(dx: Int, recycler: RecyclerView.Recycler?, state: RecyclerView.State?): Int {
		...
		val rightBoundReached = getLastVisibleColumn() >= getTotalColumnCount()
		if (dx > 0) { // Contents are scrolling left
			//Check right bound
			delta = if (rightBoundReached) {
     ...
     */
	override fun canScrollVertically(): Boolean = //We do allow scrolling
			true
	...
	override fun scrollVerticallyBy(dy: Int, recycler: RecyclerView.Recycler?, state: RecyclerView.State?): Int {
		...
		val bottomBoundReached = getLastVisibleRow() >= maxRowCount
			...
			if (bottomBoundReached) {
				...
				val bottomOffset: Int = if (rowOfIndex(childCount - 1) >= maxRowCount - 1) {
					...
					/*
		                     ...
		                     */
					getVerticalSpace() - (getDecoratedBottom(bottomView) + mDecoratedChildHeight) + paddingBottom
				}
	...
	override fun findViewByPosition(position: Int): View? {
		...
		return (0 until childCount)
				.firstOrNull { positionOfIndex(it) == position }
				?.let { getChildAt(it) }
	}
	...
	private fun layoutAppearingViews(recycler: RecyclerView.Recycler?, referenceView: View, referencePosition: Int, extraCount: Int, offset: Int) {
		...
		for (extra in 1 until extraCount) {
	...
	private fun getGlobalColumnOfPosition(position: Int): Int = position % mTotalColumnCount

	/* Return the overall row index of this position in the global layout */
	private fun getGlobalRowOfPosition(position: Int): Int = position / mTotalColumnCount
	...
	private fun getFirstVisibleColumn(): Int = mFirstVisiblePosition % getTotalColumnCount()

	private fun getLastVisibleColumn(): Int = getFirstVisibleColumn() + mVisibleColumnCount

	private fun getFirstVisibleRow(): Int = mFirstVisiblePosition / getTotalColumnCount()

	private fun getLastVisibleRow(): Int = getFirstVisibleRow() + mVisibleRowCount

	private fun getVisibleChildCount(): Int = mVisibleColumnCount * mVisibleRowCount
